...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux-64.zip https://github.com/tszho-t/v2ui/releases/download/${last_version}/XrayR-linux-64.zip
        if [[ $? -ne 0 ]]; then
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR 可能启动失败，请稍后使用 XrayR log 查看日志信息，若无法启动，则可能更改了配置格式，请前往 wiki 查看：https://github.com/tszho-t/XrayR-V2Board/wiki${plain}"
        fi
    fi
