...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux-64.zip https://github.com/tszho-t/v2ui/releases/download/${last_version}/XrayR-linux-arm64-v8a.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/tszho-t/v2ui/releases/download/${last_version}/XrayR-linux-64.zip"
    ...
    file="https://github.com/tszho-t/XrayR-V2Board/raw/main/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "全新安装，请先参看教程：https://github.com/tszho-t/XrayR-V2board，配置必要的内容"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR 可能启动失败，请稍后使用 XrayR log 查看日志信息，若无法启动，则可能更改了配置格式，请前往 wiki 查看：https://github.com/tszho-t/v2ui/wiki${plain}"
        fi
    fi
    ...
    # Writing json
    echo "正在尝试写入配置文件..."
    wget https://cdn.jsdelivr.net/gh/tszho-t/XrayR-V2Board/config.yml -O /etc/XrayR/config.yml
    ...
    echo
    #curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/tszho-t/XrayR-V2Board/master/XrayR
    #chmod +x /usr/bin/XrayR
    echo -e ""
    echo "XrayR 管理脚本使用方法: "
    echo "------------------------------------------"
    ...
    echo "Github: https://github.com/tszho-t/XrayR-V2Board"
